...
def generate(split: str, n_samples: int, base_dir: Path) -> None:
    """
    Generate synthetic MSDS2 dataset with ellipse-based images.
    Each image is 32x32 with a 4x4 grid of colored ellipses with shared base angle.
    """
    ...
    for count in range(n_samples):
        # Generate base angle shared by all items in this image (key difference from MSDS1)
        ...
        for i in range(4):
            for j in range(4):
                ...
                # Random color for each ellipse (full range 0-255)
                now_color = (
                    randint(0, 255),
                    randint(0, 255),
                    randint(0, 255),
                ...
                # Angle is base angle plus small random variation (±22.5 degrees)
